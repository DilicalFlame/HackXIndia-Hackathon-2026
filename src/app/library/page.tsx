...
const LibraryPageContent = ({ searchParams }: { searchParams: ReadonlyURLSearchParams | null }) => {
  ...
  const importBooks = async (files: SelectedFile[], groupId?: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? errorMap.find(([str]) => (error as Error).message.includes(str))?.[1] ||
    ...
    for (let i = 0; i < files.length; i += concurrency) {
      ...
      const importedBooks = (await Promise.all(batch.map(processFile))).filter(
  ...
  const handleImportBooksFromDirectory = async () => {
    ...
    const toImportFiles = await Promise.all(
      supportedFiles.map(async (file) => {
        return {
          path: await joinPaths(importDirectory as string, file.path),
