...
import {
  ...
  HackXIndia26_CHANGELOG_FILE,
  HackXIndia26_UPDATER_FILE,
...
export const checkForAppUpdates = async (
  ...
  if (['macos', 'windows', 'linux'].includes(OS_TYPE)) {
    ...
    try {
      const response = await fetch(HackXIndia26_UPDATER_FILE, { connectTimeout: 5000 });
...
export const checkAppReleaseNotes = async (isAutoCheck = true) => {
  ...
  if ((lastShownVersion && semver.gt(currentVersion, lastShownVersion)) || !isAutoCheck) {
    try {
      ...
      const res = await fetchFunc(HackXIndia26_CHANGELOG_FILE);
      if (res.ok) {
