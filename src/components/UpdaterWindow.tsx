...
import { HackXIndia26_UPDATER_FILE, HackXIndia26_CHANGELOG_FILE } from '@/services/constants';
...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      const response = await fetch(HackXIndia26_UPDATER_FILE);
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `HackXIndia26_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
    ...
    const checkWindowsPortableUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const exeFileName = `HackXIndia26_${data.version}_${arch}-portable.exe`;
        ...
        setUpdate({
          ...
          downloadAndInstall: async (onEvent) => {
            ...
            try {
              ...
              const command = Command.create('start-HackXIndia26', ['/C', 'start', '', exeFilePath]);
              await command.spawn();
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(HackXIndia26_CHANGELOG_FILE);
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of HackXIndia26 is available!')}
              </h2>
              <p className='mb-2'>
                {_('HackXIndia26 {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=com.bilingify.HackXIndia26'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in HackXIndia26")}
