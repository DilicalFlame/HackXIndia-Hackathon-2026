...
declare global {
  interface Window {
    __HackXIndia26_UPDATER_DISABLED?: boolean;
    __HackXIndia26_IS_EINK?: boolean;
  }
}
  ...
  async readDir(path: string, base: BaseDir) {
    ...
    if (!baseDir || baseDir === 0) {
      try {
        ...
        console.warn('Rust read_dir failed, falling back to JS recursion');
      }
    }
...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'HackXIndia26') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override isEink = Boolean(window.__HackXIndia26_IS_EINK);
  ...
  override hasUpdater =
    ...
    !window.__HackXIndia26_UPDATER_DISABLED;
  ...
  async migrate20251029() {
    ...
    const oldDir = await join(rootPath, 'Images', 'HackXIndia26', 'Images');

    await copyFiles(this, oldDir, newDir);

    const dirToDelete = await join(rootPath, 'Images', 'HackXIndia26');
