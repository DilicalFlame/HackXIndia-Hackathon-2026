...
export abstract class BaseAppService implements AppService {
  ...
  distChannel = 'HackXIndia26' as DistChannel;
  ...
  private async safeLoadJSON<T>(filename: string, base: BaseDir, defaultValue: T): Promise<T> {
    ...
    // If main file doesn't exist, and backup doesn't exist, it's a new install
    if (!(await this.fs.exists(filename, base)) && !(await this.fs.exists(backupFilename, base))) {
      console.log(`New install detected or config missing: ${filename}. Using default.`);
      return defaultValue;
    }
